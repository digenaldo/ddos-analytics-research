...
for bar, mean, lower, upper in zip(bars, mean_acuracias, ci_lower_acuracias, ci_upper_acuracias):
    height = bar.get_height()
    plt.text(bar.get_x() + bar.get_width() / 2, upper, f'{upper:.7f}', ha='center', va='bottom', color='black', fontsize=10, rotation=90)
    plt.text(bar.get_x() + bar.get_width() / 2, lower, f'{lower:.7f}', ha='center', va='top', color='black', fontsize=10, rotation=90)
...
# Plotar gráfico de linha com preenchimento para intervalos de confiança da acurácia
plt.figure(figsize=(12, 6))
plt.plot(rodadas, mean_acuracias, marker='o', color='green', label='Acurácia Média')

# Preencher entre as bandas de confiança superior e inferior
plt.fill_between(rodadas, ci_lower_acuracias, ci_upper_acuracias, color='lightgreen', alpha=0.3, label='Intervalo de Confiança')
